...
function trimMessages(contact) {
  ...
  console.log(
    'Trimming messages for contact: ',
    contact.id,
    ' with messages: ',
    contact.messages
  );

  let totalChars = contact.messages.reduce(
    (sum, msg) => sum + msg.content.length,
    0
  );
  let systemMessageIndex = contact.messages.findIndex(
    (m) => m.role === 'system'
  );
  ...
  while (totalChars > 4000 && contact.messages.length > 1) {
    ...
    totalChars = contact.messages.reduce(
      (sum, msg) => sum + msg.content.length,
      0
    );
  }
...
client.on('ready', async () => {
  ...
  for (const contact of contacts) {
    ...
    fullContacts.push({
      ...
      category:
        fullcontact.isBusiness || fullcontact.isEnterprise
          ? 'Business'
          : 'Private',
      messages: [],
    });
  }
  global.mainWindow.webContents.send('contacts-data', {
    fullContacts,
    llmContacts,
  });
});
